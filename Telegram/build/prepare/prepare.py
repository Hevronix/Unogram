...
win:
    ...
    curl.exe -L --fail --output ./msys64.exe https://github.com/msys2/msys2-installer/releases/download/2025-08-30/msys2-base-x86_64-20250830.sfx.exe
    msys64.exe
    del msys64.exe
...
win:
    mkdir NuGet
    curl.exe -L --fail --output ./NuGet/nuget.exe https://dist.nuget.org/win-x86-commandline/latest/nuget.exe
""", 'ThirdParty')

stage('jom', """
win:
    curl.exe -L --fail --output ./jom.zip https://qt.mirror.constant.com/official_releases/jom/jom_1_1_3.zip
...
win:
    curl.exe -L --fail --output ./gas-preprocessor.zip https://codeload.github.com/FFmpeg/gas-preprocessor/zip/refs/heads/master
    powershell -Command "Expand-Archive ./gas-preprocessor.zip -DestinationPath ."
    ren gas-preprocessor-master gas-preprocessor
    del gas-preprocessor.zip
    cd gas-preprocessor
...
if not (win and os.environ.get('UNOGRAM_SKIP_BREAKPAD') == '1'):
    stage('breakpad', """

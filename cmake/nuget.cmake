...
function(nuget_add_package package_name package package_version)
    ...
    if (NOT package_version_defined)
        ...
        set(package_dst ${packages_loc}/${package})

        set_property(GLOBAL PROPERTY ${package_key} ${package_version})
        if (EXISTS ${package_dst})
            message("Using existing NuGet package: ${package_dst}")
        else()
            string(TOLOWER "${package}" package_lower)
            set(nuget_cache_dir "$ENV{USERPROFILE}/.nuget/packages/${package_lower}/${package_version}")
            if (EXISTS ${nuget_cache_dir})
                message("Using cached NuGet package: ${nuget_cache_dir}")
                execute_process(
                COMMAND
                    ${CMAKE_COMMAND}
                    -E
